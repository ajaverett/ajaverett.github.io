        ...
        <script>
            ...
            function renderBanners() {
                eventsData.forEach(event => {
                    ...
                    bannerBar.className = 'absolute top-0 h-[8px] bg-[#2c1810]/10 border-b border-[#2c1810]/20 z-0 pointer-events-none';
                    bannerBar.style.left = `${startX}px`;
                    bannerBar.style.width = `${width}px`;
                    
                    // 2. Small Labels on the Bar itself (Start/End context)
                    const labelDiv = document.createElement('div');
                    labelDiv.className = 'absolute top-2 left-2 text-[10px] font-bold font-lato text-[#2c1810]/30 uppercase tracking-widest';
                    labelDiv.innerText = `${event.title} (START)`;
                    
                    bannerBar.appendChild(labelDiv);
                    elements.eventsContainer.appendChild(bannerBar);

                    // Save pre-calculated positions for the loop
